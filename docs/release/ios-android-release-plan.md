...
| iOS compilation/upload | In internal TestFlight | App Store Connect processed both `1.0 (1)` and new-icon `1.0 (2)` as valid. Version `1.0` is linked to build `2` for a future App Review submission, and the **Internal Testing** group includes build `2`. Confirm installation on a device. |
...
| Store records | iOS listing partly filled | App Store Connect confirms `Wroclaw Storywalk` / `com.event.horizon.wroclaw`. Travel/Education categories and EN/RU/PL listing and TestFlight text are set; see `app-store-connect-status.md`. Build `2` is linked to `Internal Testing`; owner-confirmed content rights and the free App Store price are saved. Complete App Privacy; create the Android record later. |
...
| Data practices | Needs review | Firebase Auth/Firestore can store user ID, optional email and quest progress; account creation also stores a public IP obtained from ipify. Complete the App Privacy questionnaire from these flows and verify vendor practices. |
| Store assets and contacts | In progress | The new Wroclaw icon is in processed build `1.0 (2)`. All 18 EN/RU/PL iPhone/iPad browser screenshots are uploaded and processed by Apple; compare them with native rendering before review. Privacy/support URLs and contact fields are set; a dedicated support page and licensed media inventory remain. |
...
4. **Signed archives — done for `1.0 (1)` and `1.0 (2)`.** Xcode automatic signing used Team `AB8LFY64PF`; the archives have the expected bundle ID, display name, version and embedded provisioning profile. Keep archives and diagnostics outside Git; validate runtime and icon on device.
5. **Upload and processing — done for both builds.** App Store Connect API reports both builds as `VALID`. The current script exports/uploads through `xcodebuild -exportArchive`; use `./build-ios.sh` for the next build number.
6. **Internal testing — build `2` assigned.** The `Internal Testing` group now includes builds `1.0 (1)` and `1.0 (2)`; two testers were in `INVITED` state at the prior check. Confirm they receive and accept invitations, install through TestFlight, and provide crash/route/audio/localization feedback. Internal testing precedes any external TestFlight group; external testing may need Beta App Review.
7. **App Store release later.** Compare uploaded screenshots with the native app, then finish App Privacy, accessibility, dedicated support information, availability, and a final content review. The price is already Free. Submit a tested build to App Review only after owner review of the public listing.

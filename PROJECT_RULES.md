...
## 🌐 Автоматическая проверка в браузере (BrowserMCP)

### Обязательная проверка каждого промпта
**Каждый промпт ДОЛЖЕН завершаться автоматической проверкой результата в браузере!**

#### Шаги проверки:
1. **Открыть страницу** через BrowserMCP: `mcp_browsermcp_browser_navigate`
2. **Сделать снимок страницы** для анализа: `mcp_browsermcp_browser_snapshot`
3. **Проверить результат** - убедиться, что изменения применились корректно
4. **Зафиксировать результат** в ответе пользователю

#### Пример проверки:
```javascript
// После внесения изменений в код:
1. Открыть страницу: mcp_browsermcp_browser_navigate(url)
2. Сделать снимок: mcp_browsermcp_browser_snapshot()
3. Проанализировать результат
4. Сообщить пользователю о статусе
```

#### Исключения:
- Только если BrowserMCP недоступен или не работает
- В этом случае явно указать причину и попросить пользователя проверить вручную

### Проверка функциональности
После каждого изменения проверять:
- ✅ **Загрузка страницы** - страница открывается без ошибок
- ✅ **Отображение элементов** - новые/измененные элементы видны
- ✅ **Интерактивность** - кнопки, ссылки работают
- ✅ **Стили** - CSS изменения применились
- ✅ **JavaScript** - функциональность работает

### Отчет о проверке
Каждый ответ должен содержать:
1. **Описание изменений** - что было сделано
2. **Результат проверки** - что показал браузер
3. **Статус** - успешно/требует доработки
4. **Скриншот** - если требуется показать результат

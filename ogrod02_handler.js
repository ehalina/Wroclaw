...
function switchToTownMusic() {
    ...
    if (musicIframe && musicIframe.contentWindow && musicIframe.contentWindow.musicAPI) {
        try {
            ...
            console.log('🏘️ Состояние звука:', { isMuted, soundMuted: localStorage.getItem('soundMuted') });
            
            // Создаем новый iframe с town.mp3
            ...
            // Создаем HTML для iframe с town.mp3
            const iframeHTML = `
                <!DOCTYPE html>
                <html>
                <head>
                    <meta charset="UTF-8">
                    <title>Town Music Player</title>
                </head>
                <body>
                    ...
                    <script>
                        ...
                        console.log('🏘️ iframe: восстановление состояния', { savedTime, savedPaused });
                        ...
                        if (savedPaused !== '1' && !soundMuted) {
                            console.log('🏘️ iframe: пытаемся запустить музыку town');
                            audio.play().catch((error) => {
                                console.log('🏘️ iframe: ошибка воспроизведения', error);
                            });
                        } else {
                            console.log('🏘️ iframe: музыка была на паузе или звук отключен, не запускаем');
                        }
                        ...
                        window.musicAPI = {
                            play: () => {
                                console.log('🏘️ iframe API: play вызван');
                                return audio.play();
                            },
                            pause: () => {
                                console.log('🏘️ iframe API: pause вызван');
                                return audio.pause();
                            },
                            setVolume: (vol) => {
                                console.log('🏘️ iframe API: setVolume', vol);
                                audio.volume = vol;
                            },
                            setMuted: (muted) => {
                                console.log('🏘️ iframe API: setMuted', muted);
                                audio.muted = muted;
                            },
                            getCurrentTime: () => audio.currentTime,
                            setCurrentTime: (time) => { 
                                console.log('🏘️ iframe API: setCurrentTime', time);
                                audio.currentTime = time; 
                            },
                            isPaused: () => audio.paused
                        };
                        console.log('🏘️ iframe API создан');
                    </script>
                </body>
                </html>
            `;
            ...
            console.log('🏘️ iframe с town.mp3 создан и добавлен в DOM');
            ...
            newIframe.addEventListener('load', () => {
                setTimeout(() => {
                    if (newIframe.contentWindow && newIframe.contentWindow.musicAPI) {
                        ...
                        console.log('🏘️ Глобальные функции обновлены для town.mp3');
                    }
                }, 100);
            });
            
        } catch (error) {
            console.error('🏘️ Ошибка при переключении музыки:', error);
        }
    } else {
        console.log('🏘️ iframe API недоступен, ждем...');

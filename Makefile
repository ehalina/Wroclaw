...
.PHONY: help install dev build start lint fix-lint typecheck test test-watch test-e2e smoke security security-fix audit clean reinstall doctor poc-template stage-06-08-screenshots stage-06-14-audio cap-sync cap-copy cap-add-android cap-add-ios cap-open-android cap-open-ios android-debug
...
stage-06-14-audio:
	E2E_PORT=$(E2E_PORT) npx playwright test --config=playwright.stage-06-14.config.mjs

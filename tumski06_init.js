...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    Promise.all([
        ...
        const bogomater = document.getElementById('bogomater_quest');
        if (bogomater && questMod && typeof questMod.setupQuestGeoMarker === 'function') {
            questMod.setupQuestGeoMarker({
                markerId: 'bogomater_quest',
                questNumber: parseInt(bogomater.getAttribute('data-quest-number')),
                questImage: bogomater.getAttribute('data-quest-image')
            });
            ...
            try {
                ...
                const bogomaterMarker = document.getElementById('bogomater_quest');
                if (bogomaterMarker) {
                    bogomaterMarker.classList.add('quest-marker-glow');
                }
            } catch (e) {
                // no-op
            }
    ...
    if (window.innerWidth <= 700) {
        setTimeout(() => {
            const cursors = document.querySelectorAll('.custom-cursor-back, .custom-cursor-backarea, .custom-cursor-left, .custom-cursor-leftarea');

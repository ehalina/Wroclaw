...
- [x] Verify the public Privacy Policy URL and fill EN/RU/PL policy, support and TestFlight feedback fields plus App Review/Beta Review contact details through the API (2026-09-22).
...
- [ ] Review generated icon on devices, EN/RU/PL content and submitted listing text, privacy/data practices, age rating, candidate screenshots, and support contacts before App Store submission. The current TestFlight `1.0 (1)` predates the new icon; add the local App Store Connect issuer ID under `___keys/ios/`, then run `IOS_BUILD_NUMBER=2 ./build-ios.sh` to upload the existing `1.0 (2)` archive for final validation.
...
- [x] **Fix: runtime console noise after `www/` migration** - Quieted optional tumski init, safe step audio, favicon and stale iframe timeout cleanup
  - Completed: 2026-06-27
